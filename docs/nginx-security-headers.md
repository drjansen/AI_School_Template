...
This document describes the recommended Nginx-level security headers for the
Eldarin Institute production server. These headers **complement** the
...
server {
    ...
    server_name eldarin.ai www.eldarin.ai;

    ssl_certificate     /etc/letsencrypt/live/eldarin.ai/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/eldarin.ai/privkey.pem;
...
- **Testing headers** — Use [securityheaders.com](https://securityheaders.com)
  or `curl -I https://eldarin.ai` to verify headers are being sent correctly
  after deployment.
